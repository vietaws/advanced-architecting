...
async function deleteImageFromEFS(filename) {
  ...
  try {
    ...
    logger.warn(
      { action: 'provider.image_delete', filename, error: err.message },
      'Could not delete provider image from EFS'
    );
  }
}
...
router.post('/', upload.single('image'), async (req, res) => {
  try {
    ...
    logger.info(
      { action: 'provider.create', source: 'rds', id, name, city, has_image: !!image_filename, latency_ms },
      'Provider created'
    );
...
// GET /image/:filename — serve provider image from EFS
router.get('/image/:filename', async (req, res) => {
  try {
    ...
    logger.error(
      { action: 'provider.image_serve', filename: req.params.filename, error: error.message },
...
router.get('/:id', async (req, res) => {
  try {
    ...
    logger.info(
      { action: 'provider.get', source: 'rds', id: req.params.id, found: !!result.rows[0], latency_ms },
      'RDS query'
    );
    ...
    logger.error(
      { action: 'provider.get', source: 'rds', id: req.params.id, error: error.message },
...
router.put('/:id', async (req, res) => {
  try {
    ...
    logger.info(
      { action: 'provider.update', source: 'rds', id: req.params.id, name, city, latency_ms },
      'Provider updated'
    );
    ...
    logger.error(
      { action: 'provider.update', id: req.params.id, error: error.message },
...
// DELETE /:id — delete provider + EFS image
router.delete('/:id', async (req, res) => {
  try {
    ...
    logger.info(
      { action: 'provider.delete', source: 'rds', id: req.params.id, latency_ms },
      'Provider deleted'
    );
    ...
    logger.error(
      { action: 'provider.delete', id: req.params.id, error: error.message },
